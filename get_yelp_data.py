...
def filter_yelp_response(response:dict) -> dict:
    if len(response["businesses"]) != 0:
        ...
        logging.info(f'Filtered Yelp response. Result: {filtered_yelp_response}.')
        return filtered_yelp_response
    else:
        logging.warning(f'Yelp Search API did not return any businesses.')
...
def get_yelp_data(restaurant:dict) -> dict:
    ...
    if response.status_code == 200:
        ...
        logging.warning(f'Unsuccessful call to Yelp API. Status code: {response.status_code}. {response.reason}.')
